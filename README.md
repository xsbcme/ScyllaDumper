...
Windows x64 内核级进程转存工具，功能对标 Scylla（Imports Reconstructor）。借助 KDMapper 无签名手动映射内核驱动，以内核权限执行内存读写与进程操作，绕过 Anti-Cheat / EDR 对用户态 API 的监控，从受保护进程中提取完整内存映像并重建可运行的 PE 文件。
